 ...
 * Call: POST /v1/setup  with header  x-setup-secret: <your-secret>
...
const CREATE_TABLES_SQL = [
    ...
    `CREATE TABLE IF NOT EXISTS "leads" (
        ...
        "native_address" text,
        "high_commission_name" text,
        "embassy_name" text,
        "india_address" text,
        "indian_phone_number" text,
        "tentative_duration" text,
    ...
    `CREATE TABLE IF NOT EXISTS "appointments" (
        ...
        "wa_confirmation_sent" integer DEFAULT 0,
        "notes" text,
        "created_at" integer DEFAULT (strftime('%s', 'now') * 1000)
    )`,
...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        for (const tableSql of CREATE_TABLES_SQL) {
            try {
                await db.run(sql.raw(tableSql));
                const name = tableSql.match(/CREATE TABLE IF NOT EXISTS "(\w+)"/)?.[1];
        ...
        const existingUsers = await db.run(sql.raw('SELECT COUNT(*) as c FROM users'));
        const count = existingUsers?.rows?.[0]?.[0] || 0;

        if (count === 0) {
            const passwordHash = await bcrypt.hash('Admin@123', 10);
            const userId = crypto.randomUUID();
            await db.run(sql.raw(
                `INSERT INTO users (id, name, email, password_hash, role, is_active, can_manage_users)
                 VALUES ('${userId}', 'Biswajit Saha', 'biswajit_saha@easyheals.com',
                 '${passwordHash}', 'owner', 1, 1)`
            ));
            userSeeded = true;
        }
